...
class Button:
    ...
    def show(self, gr=None):
        ...
        if self.name != '':
            ...
            self.screen.blit(b_text, (self.x + self.hx * 0.61 - len(self.name) * 13, self.y + self.hy * 0.2))
        return flag
...
class Text:
    def __init__(self, text, x, y, color, screen):
        ...
        self.color, self.font = color, pg.font.Font(None, 30)
...
if game_loop:
    ...
    while game_loop:
        ...
        if not game or my_step == my_step_b:
            ...
            my_step = my_step_b
        if not left_panel_2.mode and not left_panel_2.mode and timer2.tk() and not game:
        ...
        for event in pg.event.get():
            if event.type == pg.MOUSEBUTTONDOWN and win:
                if close_b_f:
                    game_loop = False
            if event.type == pg.MOUSEBUTTONDOWN and not win:
                ...
                if left_panel_1.mode and True in buttons[:5] and not game and flag:
                    ...
                    now.hide(False)
                elif my_step and not left_panel_1.mode and a2 and not game:
                    player.set_ship(a1, a3)
                elif my_step and not left_panel_1.mode and not a2 and None not in pxy and not game:
        ...
        if not win and not my_step and my_step == my_step_b and timer3.tk():
            if len(enemy_var):
                ...
                if rez2 == 0:
                    ...
                    enemy_po = [randint(1, 17), randint(1, 17)]
